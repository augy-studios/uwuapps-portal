...
## Step 1 — Replace `YOUR_DOMAIN` in twa-manifest.json

Open `android/twa-manifest.json` and replace every `YOUR_DOMAIN` with your live
domain (e.g. `portal.uwuapps.org`).

Or set an env var and let the build script do it:
...
└── android/
    ├── twa-manifest.json          ← Bubblewrap config (edit YOUR_DOMAIN)
