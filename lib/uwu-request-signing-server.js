...
export async function verifySignedRequest(req, supabase) {
    ...
    const path = req.url;
    const hasBody = req.body && (typeof req.body === 'string' ? req.body.length > 0 : Object.keys(req.body).length > 0);
    const bodyStr = hasBody ? (typeof req.body === 'string' ? req.body : JSON.stringify(req.body)) : '';
